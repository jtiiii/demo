...
CREATE TABLE IF NOT EXISTS t_company(
        ...
        UNIQUE (name),
        INDEX( parent_id )
) ENGINE = INNODB DEFAULT COLLATE = UTF8MB4_BIN COMMENT '公司表';
...
) ENGINE = INNODB DEFAULT COLLATE = UTF8MB4_BIN COMMENT '公司父级关系表';
...
CREATE TABLE IF NOT EXISTS t_company_path(
         ...
         UNIQUE( path ),
         FOREIGN KEY (id) REFERENCES t_company(id)
) ENGINE = INNODB DEFAULT COLLATE = UTF8MB4_BIN COMMENT '公司的路径表';